...
export default function QuoteForm({ mode }: { mode: "orcamento" | "pedido" }) {
  ...
  function clearForm() {
    setEditingId(null);
    setNumber(nextQuoteNumber(quotes, numberPrefix));
    setStatus(isPedido ? "aprovado" : "rascunho");
  ...
  const save = useCallback(() => {
    ...
    const data: Quote = {
      ...
      number: number || nextQuoteNumber(quotes, numberPrefix),
  ...
  function removeQuote() {
    if (!editingId) return;
    if (!confirm(`Excluir este ${isPedido ? "pedido" : "orçamento"} permanentemente?`)) return;
    setQuotes((prev) => prev.filter((q) => q.id !== editingId));
    clearForm();
    showToast(`${docLabel} excluído.`);
  }
      ...
      <div className="p-5">
        ...
        <div className="flex justify-between items-center gap-2.5 mb-1">
          ...
          <input
            ...
            placeholder={`${numberPrefix}-2026-000`}
          />
        </div>
        ...
        <div className="flex gap-2 mb-4 min-w-0">
          ...
          >
            <option value="">— Selecionar {isPedido ? "pedido" : "orçamento"} salvo —</option>
            {visibleQuotes.map((q) => (
